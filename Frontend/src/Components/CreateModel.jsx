            ...
            <div className="w-11/12 max-w-5xl h-[85vh] bg-white border rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-hidden">
                    <AnimatePresence mode="wait">
                        {!selectedFile ? (
                            <motion.div key="drop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full flex flex-col items-center justify-center gap-10" onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}>
                                <div>
                                    <ImageIcon className="h-40 w-40 -ml-8 -rotate-3 text-gray-600" fill="white" />
                                    <SquarePlay className="h-40 w-40 ml-8 -mt-30 rotate-3 text-gray-600" fill="white" />
                                </div>
                                <h1 className="text-xl font-medium text-gray-700">Drag and drop photos or videos here</h1>
                                <label htmlFor="files" className="bg-blue-500 text-white px-5 py-2 rounded-lg cursor-pointer hover:bg-blue-600 transition">Select from computer</label>
                                <input type="file" id="files" accept="image/*,video/*" className="hidden" ref={fileInputRef} onChange={handleFileChange} />
                            </motion.div>
                        ) : step === "Crop" ? (
                            <motion.div key="crop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative h-full w-full">
                                <Cropper image={selectedFile} crop={crop} zoom={zoom} aspect={1} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={handleCropComplete} cropShape="rect" showGrid={false} />
                                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-10/12">
                                    <input type="range" min={1} max={3} step={0.1} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="w-full accent-blue-500 cursor-pointer" />
                                </div>
                            </motion.div>
                        ) : (
                            <motion.div key="filter" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative h-full w-full flex">
                                <div className="relative flex-[2]">
                                    <img src={step === "Filter" ? croppedFile : preview} alt="Cropped Preview" className="h-full w-full object-cover rounded-bl-lg" style={step === "Filter" ? { filter: filters[selectedFilter] } : null} />
                                </div>
                                {step === "Filter" ? (
                                    <div className="flex-1 flex flex-col gap-3 overflow-y-auto p-3 pb-10 max-h-[calc(100vh-150px)]">
                                        {Object.keys(filters).map((name) => (
                                            <button key={name} onClick={() => setSelectedFilter(name)} className={`py-2 rounded-lg cursor-pointer border text-sm font-medium transition ${selectedFilter === name ? "bg-blue-500 text-white border-blue-600" : "bg-gray-100 hover:bg-gray-200 border-gray-300"}`}>{name}</button>
                                        ))}
                                    </div>
                                ) : (
                                    <div className="flex-1 flex flex-col items-center p-2">
                                        <label htmlFor="content" className="text-sm font-medium text-gray-600 mb-2">Add Caption</label>
                                        <textarea id="content" className="flex-1 max-h-50 w-full p-2 rounded-md border resize-none focus:ring-2 focus:ring-blue-400 focus:outline-none" value={postData.content} onChange={(e) => setPostData({ ...postData, content: e.target.value })} placeholder="Write something..." disabled={isPending} />
                                    </div>
                                )}
                            </motion.div>
                        )}
