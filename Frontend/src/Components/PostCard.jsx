    ...
    return (
        <div className="space-y-4 sm:space-y-6 pb-6">
            {allPosts.map((post) => {
                ...
                return (
                    <div key={post._id} className="bg-white rounded-2xl shadow-sm border border-gray-200">
                        ...
                        <div className="p-3 sm:p-4">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center space-x-3 sm:space-x-4">
                                    <button onClick={() => toggleLikeToPost(post._id)} disabled={likePending} className={`flex items-center cursor-pointer active:scale-90 ${hasLiked ? "text-red-500" : "hover:text-gray-600"}`}>
                                        <Heart className="w-6 h-6 sm:w-7 sm:h-7" fill={hasLiked ? "red" : "none"} />
                                    </button>
                                    <Link to={`/post/${post._id}`} className="hover:text-gray-600 active:scale-90"><MessageCircle className="w-5 h-5 sm:w-6 sm:h-6" /></Link>
                                    <button className="hover:text-gray-600 cursor-pointer active:scale-90"><SendHorizontal className="w-5 h-5 sm:w-6 sm:h-6" /></button>
                                </div>
                                <button className="hover:text-gray-600 cursor-pointer active:scale-90"><Bookmark className="w-5 h-5 sm:w-6 sm:h-6" /></button>
                            </div>
                            ...
                            <div className="flex items-center gap-2 mb-2 sm:mb-3">
                                ...
                                <p className="text-xs sm:text-sm font-semibold cursor-pointer" onClick={() => { setSelectedLikes(post?.likes || []); setOpenLikeBox(true); }}>{post?.likes?.length || 0} likes</p>
                            </div>
                            ...
                            <form onSubmit={(e) => {
                                ...
                                <input name="comment" type="text" placeholder="Add a comment..." className="flex-1 bg-gray-100 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                <button type="submit" className="hover:text-gray-600 cursor-pointer"><SendHorizontal className="w-5 h-5 sm:w-6 sm:h-6" /></button>
                            </form>
                        </div>
                    </div>
                );
