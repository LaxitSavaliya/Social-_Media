    ...
    return (
        <>
            ...
            <motion.div
                ...
                className="hidden md:flex flex-col fixed top-0 left-0 h-screen z-10 w-20 xl:w-60 bg-white shadow-lg border-r border-gray-200"
            >
                <div className="flex items-center justify-center xl:justify-start gap-2 p-6 border-b border-gray-200">
                    <img src={Logo} className="w-10 h-10" alt="Logo" />
                    <h1 className="hidden xl:inline text-2xl font-bold bg-gradient-to-r from-indigo-500 to-purple-600 bg-clip-text text-transparent">Social Media</h1>
                </div>
                ...
                <div className="p-1.5 border-t mt-auto flex justify-center xl:justify-start">
                    <Link to={`/profile/${authUser?.userName}`} className={`flex items-center gap-2 xl:gap-4 hover:bg-gray-200 rounded-xl px-4 py-3 w-full ${currentPath === `/profile/${authUser?.userName}` ? 'bg-gray-300 font-semibold' : ''}`}>
                        {authUser?.profilePic ? (
                        ...
                        <div className="hidden xl:flex flex-col">
                            ...
                            <p className="text-xs text-success flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-600 inline-block" />Online</p>
                        </div>
                    </Link>
                </div>
            </motion.div>
            ...
            >
                {mobileNavItems.map((item) => {
                    ...
                    return (
                        <Link key={item.label} to={item.path} className={`flex flex-col items-center justify-center text-gray-700 ${currentPath === item.path ? 'text-blue-500' : ''}`}>{item.icon}</Link>
                    );
                })}
